...
# NOTAS:
# Git.init: UBICAR EL REPOSITORIO CORRESPONDIENTE
...
class Git:
    def init():
        # Para no tener que dejar a init con toda la responsabilidad, crear el repositorio para 'db' en blanco
        os.system('git init')
    ...
    def _push(self, commit_msg, branch):
        os.system('git add db')
        os.system(f'git commit -m \'auto_commit({commit_msg})\'')
        os.system(f'git branch -m {branch}')
        os.system(f'git push pa {branch}')

    def push(self):
        self._push('event', 'main')

    def back_up(self):
        self._push('back_up', 'bu')

    def auto_push(self, time_to_sleep=1800):
        while True:
            sleep(time_to_sleep)
            self._push('auto_bu', 'bu')

    def start_auto_back_up(self):
        self.auto_bu.start()

    def stop_auto_back_up(self):
        self.auto_bu.terminate()
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                if e in ('__EXIT__', None):
                    if e == '__EXIT__':
                        self.save()
                    git.stop_auto_back_up()
                    git.back_up()
                    self.close()
                    break
